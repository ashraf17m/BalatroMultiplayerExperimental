...
local function ban_and_remove_shop_joker(joker_key)
	G.GAME.banned_keys[joker_key] = true
	if not (G.shop_jokers and G.shop_jokers.cards) then return end

	for i = #G.shop_jokers.cards, 1, -1 do
		local shop_card = G.shop_jokers.cards[i]
		if shop_card.config.center.key == joker_key then
			shop_card.T.r = -0.2
...
SMODS.Joker({
	...
	add_to_deck = function(self, card, from_debuff)
		ban_and_remove_shop_joker("j_mp_idol_sandbox_collector")
	end,
	...
	calculate = function(self, card, context)
		...
		if
			...
			and MP.GRADIENT.matches_rank(context.other_card, zealot.id)
		then
			return {
...
SMODS.Joker({
	...
	add_to_deck = function(self, card, from_debuff)
		ban_and_remove_shop_joker("j_mp_idol_sandbox_zealot")
	end,
	loc_vars = function(self, info_queue, card)
	...
	calculate = function(self, card, context)
		...
		if
			context.individual
			and context.cardarea == G.play
			and MP.GRADIENT.matches_rank(context.other_card, most_common_card.id)
