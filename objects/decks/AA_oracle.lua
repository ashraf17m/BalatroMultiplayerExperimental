MP.ORACLE = MP.ORACLE or {}
local oracle = MP.ORACLE

function oracle.apply_dollar_cap(mod, current_dollars, max_dollars)
	return math.min(max_dollars - current_dollars, mod)
end

function oracle.should_show_max(mod, current_dollars, max_dollars)
	return mod == 0 and current_dollars >= max_dollars
end

function oracle.show_max_alert(dollar_UI)
	attention_text({
		text = "MAX",
		scale = 0.8,
		hold = 0.7,
