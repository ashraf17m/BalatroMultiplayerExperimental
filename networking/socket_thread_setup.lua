...
local function pushNetworkUiAction(action, message)
	return pushNetworkUiPayload(json.encode({
		action = action,
		message = message,
	}))
end
...
function Networking.connect()
	...
	if not client then
		if not isReconnecting then
			pushNetworkUiAction("error", "Failed to create multiplayer socket")
		end
		return false
	end
	...
	if connectionResult ~= 1 then
		...
		if not isReconnecting then
			pushNetworkUiAction("error", "Failed to connect to multiplayer server")
		end
		return false
	end
