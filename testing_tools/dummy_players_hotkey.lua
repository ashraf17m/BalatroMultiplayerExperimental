...
MP.TESTING_DUMMY_PLAYERS = MP.TESTING_DUMMY_PLAYERS or {
	players = {},
	counter = 0,
}

local function get_runtime()
	return MP.TESTING_DUMMY_PLAYERS
end
...
function MP.get_testing_dummy_players()
	local runtime = get_runtime()
...
local function request_dummy_player_refresh()
	if MP.GAME and MP.is_pvp_boss and MP.is_pvp_boss() then
		...
		elseif MP.UI and MP.UI.refresh_player_list then
	...
	if MP.request_lobby_overlay_refresh then
		MP.request_lobby_overlay_refresh()
	end
end

local function show_dummy_notice(player, count)
	local message = "Testing dummy: " .. tostring(player.username) .. " (" .. tostring(count) .. ")"
...
function MP.add_testing_dummy_player()
	local runtime = get_runtime()
