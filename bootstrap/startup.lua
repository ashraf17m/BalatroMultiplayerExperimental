...
local function abort_bootstrap(message)
	...
	MP.NETWORKING_INTERNAL = MP.NETWORKING_INTERNAL or {}
	MP.NETWORKING_INTERNAL.bootstrap_failed = true
	MP.NETWORKING_INTERNAL.bootstrap_failure_message = failure_message
...
local initialize_runtime_state = function()
	if MP.initialize_lobby_runtime_state then
		MP.initialize_lobby_runtime_state()
	end
...
local initialize_lobby_defaults = function()
	if not MP.reset_lobby_config or not MP.reset_game_states then
		return false
	end
...
local CORE_BOOTSTRAP_STEPS = {
	...
	{
		kind = "files",
		paths = TEAM_SYNC_FEATURE_FILES,
		label = "Team sync feature files",
		options = { required = true },
