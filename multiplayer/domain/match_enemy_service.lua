...
function MATCH_DOMAIN.apply_enemy_info(
	player_id,
	username,
	score_str,
	hands_left_str,
	...
	local score = (type(score_str) == "string" or type(score_str) == "number") and MP.INSANE_INT.from_string(score_str) or nil
	local hands_left = tonumber(hands_left_str)
	local skips = tonumber(skips_str)
	local lives = tonumber(lives_str)
	local shared_team_lives = tonumber(team_lives)
