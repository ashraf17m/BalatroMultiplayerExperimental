...
function LOBBY_DOMAIN.get_valid_gamemode(gamemode)
...
function LOBBY_DOMAIN.set_creation_gamemode(gamemode_key, state)
	return LOBBY_DOMAIN.set_setup_field("creation_gamemode", LOBBY_DOMAIN.normalize_gamemode(gamemode_key), state)
end
...
function LOBBY_DOMAIN.build_default_config(persist_ruleset_and_gamemode, lobby_type, state)
...
function LOBBY_DOMAIN.reset_config(persist_ruleset_and_gamemode, lobby_type, state)
	...
	LOBBY_DOMAIN.set_config(LOBBY_DOMAIN.build_default_config(persist_ruleset_and_gamemode, lobby_type, state), state)
...
function LOBBY_DOMAIN.prepare_config_for_creation(state)
	...
	config.gamemode = LOBBY_DOMAIN.get_valid_gamemode(config.gamemode)
	...
	local hides_lives_and_timer_hud = config.gamemode == "gamemode_mp_survival"
		or config.gamemode == "gamemode_mp_coop"

	if hides_lives_and_timer_hud then
