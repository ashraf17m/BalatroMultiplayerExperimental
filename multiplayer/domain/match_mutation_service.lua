...
function INTERNAL.normalize_nonnegative_integer(value)
	return math.max(0, math.floor(tonumber(value) or 0))
end

function INTERNAL.set_runtime_activity(state, active_field, generation_field, is_active)
	state[active_field] = not not is_active
	state[generation_field] = INTERNAL.normalize_nonnegative_integer(state[generation_field]) + 1
	return state[generation_field]
end

function INTERNAL.increment_nonnegative_counter(state, field_name, amount)
	state[field_name] = INTERNAL.normalize_nonnegative_integer(state[field_name])
		+ INTERNAL.normalize_nonnegative_integer(amount)
	return state[field_name]
end

function INTERNAL.consume_nonnegative_counter(state, field_name)
	local value = INTERNAL.normalize_nonnegative_integer(state[field_name])
	state[field_name] = 0
	return value
end
...
function MATCH_DOMAIN.set_timer_value(time, state)
	state = state or MATCH_DOMAIN.ensure_state()
	state.timer = INTERNAL.normalize_nonnegative_integer(time)
	return state.timer
end

function MATCH_DOMAIN.stop_timer_runtime(state)
	state = state or MATCH_DOMAIN.ensure_state()
	return INTERNAL.set_runtime_activity(
		state,
...
function MATCH_DOMAIN.lock_timer_for_ante(state)
	state = state or MATCH_DOMAIN.ensure_state()
	state.timer_locked_for_ante = true
	return true
end
...
function MATCH_DOMAIN.apply_timer_skip_for_ante(skip_count_delta, base_time, increment_seconds, state)
	...
	skip_count_delta = INTERNAL.normalize_nonnegative_integer(skip_count_delta)
	...
	local next_skip_count = INTERNAL.increment_nonnegative_counter(
		state,
...
function MATCH_DOMAIN.begin_pvp_countdown(seconds, state)
	state = state or MATCH_DOMAIN.ensure_state()
	state.pvp_countdown = INTERNAL.normalize_nonnegative_integer(seconds)
	return state.pvp_countdown
end

function MATCH_DOMAIN.tick_pvp_countdown(state)
	state = state or MATCH_DOMAIN.ensure_state()
	state.pvp_countdown = math.max(0, INTERNAL.normalize_nonnegative_integer(state.pvp_countdown) - 1)
	return state.pvp_countdown
end
...
function MATCH_DOMAIN.set_misprint_display(misprint_display, state)
	state = state or MATCH_DOMAIN.ensure_state()
	state.misprint_display = tostring(misprint_display or "")
	return state.misprint_display
end
...
function MATCH_DOMAIN.increment_asteroids(amount, state)
	state = state or MATCH_DOMAIN.ensure_state()
	return INTERNAL.increment_nonnegative_counter(state, "asteroids", amount)
end

function MATCH_DOMAIN.consume_asteroids(state)
	state = state or MATCH_DOMAIN.ensure_state()
	return INTERNAL.consume_nonnegative_counter(state, "asteroids")
end

function MATCH_DOMAIN.increment_pizza_discards(amount, state)
	state = state or MATCH_DOMAIN.ensure_state()
	return INTERNAL.increment_nonnegative_counter(state, "pizza_discards", amount)
end

function MATCH_DOMAIN.consume_pizza_discards(state)
	state = state or MATCH_DOMAIN.ensure_state()
	return INTERNAL.consume_nonnegative_counter(state, "pizza_discards")
end
