...
function LOBBY_DOMAIN.build_initial_client_state()
	return {
		connected = false,
		username = "Guest",
		blind_col = 1,
...
function LOBBY_DOMAIN.build_initial_setup_state()
	return {
		temp_code = "",
		temp_seed = "",
...
function LOBBY_DOMAIN.build_initial_state()
	...
	return {
		...
		config = {},
		run_deck = run_deck,
		client = LOBBY_DOMAIN.build_initial_client_state(),
		setup = LOBBY_DOMAIN.build_initial_setup_state(),
		players = {},
		is_host = false,
...
function LOBBY_DOMAIN.ensure_client_state(state)
	state = state or LOBBY_DOMAIN.ensure_state()
	state.client = state.client or LOBBY_DOMAIN.build_initial_client_state()
	return state.client
end

function LOBBY_DOMAIN.ensure_setup_state(state)
	state = state or LOBBY_DOMAIN.ensure_state()
	state.setup = state.setup or LOBBY_DOMAIN.build_initial_setup_state()
	return state.setup
end
