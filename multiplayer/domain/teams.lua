...
local function get_normalized_score_text(score_value)
	local match_state_internal = MP.MATCH_STATE_INTERNAL or {}
	local normalize_score_text = match_state_internal.normalize_score_text
	if normalize_score_text then
		return normalize_score_text(score_value)
	end

	return tostring(score_value or 0)
end
...
local function refresh_shared_score_ui_if_changed(previous_team_score_text, previous_team_lives)
	...
	if previous_team_score_text ~= MP.GAME.team_score_text or previous_team_lives ~= MP.GAME.team_lives then
		...
		elseif MP.UI and MP.UI.refresh_shared_score_ui then
...
function TEAMS_DOMAIN.is_cooperative_blind()
	...
	if MP.blind_choice_row_is_teams_cooperative_for_row then
		return MP.blind_choice_row_is_teams_cooperative_for_row(row)
	end
