...
local function stop_local_ante_timer_runtime()
	if MP.stop_match_timer_runtime then
		MP.stop_match_timer_runtime()
	end
...
local function refresh_synced_timer_display()
	...
	if MP.set_match_timer_value then
		MP.set_match_timer_value(remaining_seconds)
	else
...
local function ensure_local_ante_timer_runtime()
	...
	local runtime_generation = MP.start_match_timer_runtime and MP.start_match_timer_runtime() or MP.GAME.timer_runtime_generation
...
function ante_timer_runtime.apply_state(time, timer_started, play_sfx, server_now, deadline_at, timer_generation)
	...
	if MP.apply_match_timer_state then
		MP.apply_match_timer_state(time, timer_started)
	end
...
function ante_timer_runtime.reset_for_ante(time)
	if MP.reset_match_timer_for_ante then
		MP.reset_match_timer_for_ante(normalize_ante_timer_time(time))
	end
	clear_timer_sync()
	clear_timer_event_handle()
end

function ante_timer_runtime.lock_for_ante()
	if MP.lock_match_timer_for_ante then
		MP.lock_match_timer_for_ante()
	end
end
...
function ante_timer_runtime.apply_skip_for_ante(skip_count_delta)
	...
	local result = MP.apply_match_timer_skip_for_ante and MP.apply_match_timer_skip_for_ante(
		skip_count_delta,
...
ante_timer_runtime.start_local_ante_timer = ante_timer_runtime.start
ante_timer_runtime.pause_local_ante_timer = ante_timer_runtime.pause
...
MP.NETWORKING_INTERNAL.start_local_ante_timer = ante_timer_runtime.start_local_ante_timer
MP.NETWORKING_INTERNAL.pause_local_ante_timer = ante_timer_runtime.pause_local_ante_timer
