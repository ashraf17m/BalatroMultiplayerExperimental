...
local function get_cached_lobby_hash_info(hash_value)
	...
	if cached then
		return cached.config, cached.mod_string, cached.mod_count
	end

	local config, mod_string = MP.UTILS.parse_Hash(hash_value)
	local mod_count = 0
...
local function get_player_wire_can_manage(player_wire, is_host, is_self)
	return is_host and (not is_self) and type(player_wire) == "table" and not player_wire.isOwner
end
...
function lobby_player_snapshot.normalize_player_payload(player_wire, is_host, uses_lobby_ready)
	local username, blind_col = get_player_identity(player_wire)
	local config, mod_string, mod_count = get_cached_lobby_hash_info(player_wire.modHash)
	...
	local can_manage = get_player_wire_can_manage(player_wire, is_host, is_self)
	...
	return {
		...
		hash_str = mod_string,
		hash = MP.UTILS.hash_mod_string(mod_string),
