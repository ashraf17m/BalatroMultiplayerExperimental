...
function action_route_runtime.build_protocol_v2_lobby_routes()
	...
	return {
		...
		[build_protocol_route_key("lobby", "handyMPExtensionLobbyEnabled", LOBBY_SNAPSHOT_SCHEMA_ID)] = function(payload)
			if MP.NETWORKING_INTERNAL.handle_handy_mp_extension_lobby_enabled then
				MP.NETWORKING_INTERNAL.handle_handy_mp_extension_lobby_enabled(payload.enabled)
			end
		end,
...
function action_route_runtime.build_protocol_v2_match_routes()
	...
	return {
		...
		[build_protocol_route_key("match", "startBlind", MATCH_STATE_SCHEMA_ID)] = route_noargs("handle_start_blind"),
		[build_protocol_route_key("match", "stopGame", MATCH_STATE_SCHEMA_ID)] = route_noargs("handle_stop_game"),
		[build_protocol_route_key("match", "endPvp", MATCH_STATE_SCHEMA_ID)] = route_noargs("handle_end_pvp"),
		[build_protocol_route_key("match", "enemyInfo", MATCH_STATE_SCHEMA_ID)] = route_fields("handle_enemy_info", {
			"playerId",
			"username",
			"score",
			"handsLeft",
...
function action_route_runtime.build_protocol_v2_feature_routes()
	...
	return {
		...
		[build_protocol_route_key("endgame", "endGameStatsRequested", ENDGAME_STATE_SCHEMA_ID)] = route_noargs(
			"handle_end_game_stats_requested"
		),
		[build_protocol_route_key("endgame", "nemesisEndGameStats", ENDGAME_STATE_SCHEMA_ID)] = function()
