...
local function ensure_end_game_view_runtime()
	local ui = MP.UI or nil
	if has_required_methods(ui, END_GAME_VIEW_METHODS) then
		return ui
	end
	...
	if has_required_methods(loaded, END_GAME_VIEW_METHODS) then
		return loaded
	end
...
local function report_end_game_jokers_failure(target_id, details)
	report_end_game_view_failure(
		...
		function(end_game_view)
			end_game_view.jokers_text = localize("k_enemy_jokers") .. " (Unavailable)"
		end
	)
end
...
BALATRO.set_ui_function("load_end_game_jokers", function()
	...
	if not card_area_save then
		report_end_game_jokers_failure(target_id, string.format("Failed to unpack enemy jokers: %s", err))
		return
	end

	success, err = pcall(end_game_view.jokers_area.load, end_game_view.jokers_area, card_area_save)
	if not success then
		report_end_game_jokers_failure(target_id, string.format("Failed to load enemy jokers: %s", err))
	...
	end_game_view.jokers_text = localize("k_enemy_jokers")
...
local function action_send_game_stats()
	if not MP.GAME.stats then
		Client.send(MP.FEATURE_WIRE.build_nemesis_stats_payload({}))
		return
	end
	...
	local voucher_keys = ""
	local used_vouchers = BALATRO.get_used_vouchers and BALATRO.get_used_vouchers() or nil
	if used_vouchers then
		local keys = {}
	...
	if voucher_keys ~= "" then
		stats.vouchers = voucher_keys
	end

	Client.send(MP.FEATURE_WIRE.build_nemesis_stats_payload(stats))
end
...
function end_game_message_runtime.send_local_game_stats()
	action_send_game_stats()
end

function end_game_message_runtime.handle_end_game_stats_requested()
	action_send_game_stats()
end
...
function end_game_message_runtime.handle_receive_end_game_jokers(keys, source_player_id)
	...
	active_view.jokers_text = localize("k_enemy_jokers")
...
MP.NETWORKING_INTERNAL.send_local_game_stats = end_game_message_runtime.send_local_game_stats
MP.NETWORKING_INTERNAL.handle_end_game_stats_requested = end_game_message_runtime.handle_end_game_stats_requested
