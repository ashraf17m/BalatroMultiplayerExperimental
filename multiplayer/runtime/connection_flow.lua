...
local function ensure_server_player_id(player_id, failure_message)
	return MP.NETWORKING_INTERNAL.ensure_server_player_id(player_id, failure_message)
end
...
local function apply_server_lobby_entry_state(code, gamemode_key, lobby_type, reconnect_token, player_id, options, players, is_host, is_in_game)
	...
	local applied, result = MP.LOBBY_SESSION.apply_initial_lobby_snapshot(options, players, is_host, is_in_game)
	if not applied then
...
function connection_flow.handle_connected()
	...
	if reconnect_token and last_lobby_code then
		...
		MP.CONNECTION_WIRE.send_rejoin(last_lobby_code, reconnect_token)
	end
end

function connection_flow.handle_joined_lobby(code, gamemode_key, lobby_type, token, player_id, options, players, is_host, is_in_game)
	local reconnect_token = select(1, MP.CONNECTION_SESSION.get_reconnect_lobby_state())

	if not ensure_server_player_id(player_id, "Connection failed.") then
		return
	end

	MP.CONNECTION_RESUME.complete_manual_resume()
	apply_server_lobby_entry_state(code, gamemode_key, lobby_type, token or reconnect_token, player_id, options, players, is_host, is_in_game)
end

function connection_flow.handle_rejoined_lobby(code, gamemode_key, lobby_type, token, player_id, options, players, is_host, is_in_game)
	if not ensure_server_player_id(player_id, "Rejoin failed.") then
	...
	MP.CONNECTION_FEEDBACK.clear_self_reconnect_countdown()
	if not apply_server_lobby_entry_state(code, gamemode_key, lobby_type, token, player_id, options, players, is_host, is_in_game) then
...
function connection_flow.handle_keep_alive()
	MP.CONNECTION_WIRE.send_keep_alive_ack()
end
...
function connection_flow.handle_error(message, display)
	if MP.clear_team_money_transfer_pending then
		MP.clear_team_money_transfer_pending()
	end
