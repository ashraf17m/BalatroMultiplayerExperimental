...
function match_flow_runtime.start_match_blind_runtime()
	...
	if MP.clear_skip_ready_state then
...
function match_flow_runtime.handle_team_skip_blind_runtime(blind_row)
	...
	if MP.perform_team_skip then
		MP.perform_team_skip(blind_row)
	end
end

function match_flow_runtime.stop_match_runtime()
	MP.MATCH_LIFECYCLE.stop_match_runtime()
end
...
function match_flow_runtime.end_current_pvp_runtime()
	if MP.mark_match_end_pvp then
		MP.mark_match_end_pvp()
	end
...
MP.NETWORKING_INTERNAL.stop_match_runtime = match_flow_runtime.stop_match_runtime
