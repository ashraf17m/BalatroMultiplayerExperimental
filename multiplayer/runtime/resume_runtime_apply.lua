...
local function refresh_resumed_team_shared_score()
	...
	local is_cooperative_shared_score = (MP.is_team_cooperative_blind and MP.is_team_cooperative_blind())
		or (MP.is_coop_blind and MP.is_coop_blind())

	if is_cooperative_shared_score then
		if MP.set_match_shared_score_text then
			MP.set_match_shared_score_text(MP.GAME.team_score_text or MP.GAME.shared_score_text or "0")
		end
