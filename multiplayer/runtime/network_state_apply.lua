...
local function get_local_player_in_match(players)
	return lobby_player_snapshot.get_local_player_in_match(players)
end

local function normalize_lobby_player_wire_payload(player_wire, is_host, uses_lobby_ready)
	return lobby_player_snapshot.normalize_player_payload(player_wire, is_host, uses_lobby_ready)
end
...
function network_state_apply.lobby_info(players, is_host, is_in_game, lobby_type)
	...
	for _, player_wire in ipairs(lobby_players) do
		local player_state = normalize_lobby_player_wire_payload(player_wire, is_host, uses_lobby_ready)
	...
	if match_domain.sync_enemies_from_lobby_snapshot then
		local local_player_in_match = get_local_player_in_match(lobby_players)
...
function network_state_apply.enemy_info(
	player_id,
	username,
	score_str,
	hands_left_str,
