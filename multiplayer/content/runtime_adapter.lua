...
function content_runtime.get_nemesis_enemy_state()
	return MP.get_nemesis_enemy_state and MP.get_nemesis_enemy_state() or nil
end
...
function content_runtime.get_nemesis_skip_count()
	local enemy = content_runtime.get_nemesis_enemy_state()
	return tonumber(enemy and enemy.skips) or 0
end
...
function content_runtime.increment_asteroids(amount)
	if MP.increment_match_asteroids then
		return MP.increment_match_asteroids(amount)
	end
...
function content_runtime.increment_pizza_discards(amount)
	if MP.increment_match_pizza_discards then
		return MP.increment_match_pizza_discards(amount)
	end
...
function content_runtime.get_effective_lobby_deck()
	return MP and MP.get_effective_lobby_deck and MP.get_effective_lobby_deck() or nil
end
...
function content_runtime.persist_cocktail_config(cocktail_config)
	...
	if content_runtime.get_lobby_config_value("different_decks", false) and MP and MP.update_lobby_run_deck then
		MP.update_lobby_run_deck({
			cocktail = cocktail_config,
		})
		return true
	end

	if MP and MP.set_lobby_config_field then
		MP.set_lobby_config_field("cocktail", cocktail_config)
