...
function MP.LOBBY_WIRE.build_full_lobby_option_update()
	local full_update = {}
	for option_key, option_value in pairs(MP.LOBBY.config or {}) do
		full_update[option_key] = option_value
	end
	return full_update
end

function MP.LOBBY_WIRE.build_create_lobby_payload(gamemode)
	return build_lobby_action_payload("create", {
		gameMode = gamemode,
		lobbyType = MP.LOBBY.lobby_type,
		options = MP.LOBBY_WIRE.build_full_lobby_option_update(),
	})
end
...
function MP.LOBBY_WIRE.build_lobby_info_payload()
	return build_lobby_action_payload("requestSnapshot")
end
...
function MP.LOBBY_WIRE.build_lobby_option_sync_payload(options)
	local payload = MP.LOBBY_WIRE.build_full_lobby_option_update()
...
function MP.LOBBY_WIRE.send_lobby_option_update(options)
	if type(options) ~= "table" then
		return
	end

	if next(options) ~= nil then
		Client.send(build_match_action_payload("lobbyOptions", {
			options = options,
		}))
	end
end
