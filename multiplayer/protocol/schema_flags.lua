...
MP.PROTOCOL.SCHEMA_DEFINITIONS = SCHEMA_DEFINITIONS
MP.PROTOCOL.MESSAGE_SCHEMAS = message_schemas
MP.PROTOCOL.SCHEMA_FLAGS = schema_flags

function MP.PROTOCOL.get_schema_id(family, action)
	local family_key = tostring(family or "")
	local action_key = tostring(action or "")
	local family_schemas = MP.PROTOCOL.MESSAGE_SCHEMAS[family_key]
	return family_schemas and family_schemas[action_key] or nil
end

function MP.PROTOCOL.get_schema_flags(schema_id)
	return MP.PROTOCOL.SCHEMA_FLAGS[tostring(schema_id or "")]
end
