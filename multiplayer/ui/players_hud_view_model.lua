...
local function get_local_hands_left()
	return (G and G.GAME and G.GAME.current_round and G.GAME.current_round.hands_left) or 0
end
...
local function add_dummy_standings_players(players, included_ids)
	if not MP.get_testing_dummy_players then
		return
	end

	for _, dummy in ipairs(MP.get_testing_dummy_players()) do
