...
function MP.get_blind_choice_row_type(e)
	local el = e
	for _ = 1, 12 do
...
function MP.get_blind_choice_row_kind(e)
...
function MP.get_match_ready_blind_kind()
...
local function get_match_ready_blind_mode()
	local ready_blind_kind = MP.get_match_ready_blind_kind and MP.get_match_ready_blind_kind() or nil
...
function MP.is_readying_pvp_blind()
	return get_match_ready_blind_mode() == "pvp"
end

function MP.is_pvp_timer_context()
	if not (MP and MP.LOBBY and MP.LOBBY.config and MP.LOBBY.config.timer) then
		return false
	end
	if MP.is_readying_pvp_blind and MP.is_readying_pvp_blind() then
		return true
	end
...
function MP.blind_choice_row_is_teams_cooperative_for_row(row)
...
function INTERNAL.is_team_skip_ready_row(row)
	return MP.LOBBY
		...
		and MP.blind_choice_row_is_teams_cooperative_for_row(row)
end
