...
local function should_allow_pvp_timer_interaction()
	return MP.is_pvp_timer_context and MP.is_pvp_timer_context()
end

BALATRO.set_ui_function("mp_timer_button", function(e)
	if
		not should_allow_pvp_timer_interaction()
		or not (MP.is_readying_pvp_blind and MP.is_readying_pvp_blind())
		or MP.GAME.timer <= 0
	then
		return
	end
...
function MP.UI.start_pvp_countdown(callback)
	...
	if MP.begin_match_pvp_countdown then
		MP.begin_match_pvp_countdown(seconds)
	end
	...
	local function show_next()
		...
		if MP.tick_match_pvp_countdown then
			MP.tick_match_pvp_countdown()
		end
...
BALATRO.set_ui_function("set_timer_box", function(e)
	...
	elseif MP.is_readying_pvp_blind and MP.is_readying_pvp_blind() then
