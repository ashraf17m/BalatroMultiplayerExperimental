...
local function get_player_list_mode()
	...
	if MP.is_ffa_mode and MP.is_ffa_mode() then
		return "ffa"
	end
	return nil
end
...
local function build_player_list_signature(mode)
	...
	local parts = {
		...
		stringify_signature_value(MP.LOBBY and MP.LOBBY.config and MP.LOBBY.config.ffa_scoring_beat_average),
	}
...
local function get_ffa_full_window_max_start(players)
	local average_data = MP.UI.calculate_ffa_average(players)
	local total_entries = #players + (average_data.show_average and 1 or 0)
	return math.max(1, total_entries - MP.UI.FFA_FULL_WINDOW + 1)
end
...
function MP.UI.remove_player_list(skip_theme_restore)
	MP.UI.reset_ffa_standings_ui_state()
	local player_list_runtime = MP.UI.get_player_list_runtime()
...
local function reopen_ffa_full_standings(reset_scroll)
	local players = MP.UI.get_sorted_players()
	local state = MP.UI.get_ffa_standings_ui_state()
	local max_start = get_ffa_full_window_max_start(players)

	if reset_scroll then
		state.scroll_index = 1
	else
		state.scroll_index = math.max(1, math.min(state.scroll_index or 1, max_start))
	end
...
BALATRO.set_ui_function("mp_open_full_standings", function()
	...
	reopen_ffa_full_standings(true)
end)

BALATRO.set_ui_function("mp_ffa_standings_scroll_up", function()
	local state = MP.UI.get_ffa_standings_ui_state()
	state.scroll_index = math.max(1, (state.scroll_index or 1) - 1)
	reopen_ffa_full_standings(false)
end)

BALATRO.set_ui_function("mp_ffa_standings_scroll_down", function()
	local players = MP.UI.get_sorted_players()
	local max_start = get_ffa_full_window_max_start(players)
	local state = MP.UI.get_ffa_standings_ui_state()
	state.scroll_index = math.min(max_start, (state.scroll_index or 1) + 1)
	reopen_ffa_full_standings(false)
end)

function MP.UI.refresh_ffa_table()
