...
local function create_match_lobby_player_row(lobby_player, row_index, lobby_context, compact)
	...
	local create_surface_lane_from_spec = ROW_LAYOUT.create_surface_lane_from_spec

	local model = MP.build_match_lobby_player_row_model(lobby_player, row_index, {
		lobby_context = lobby_context,
	})
	local row_nodes = {
		create_surface_lane_from_spec(model.lives_lane_spec),
	...
	append_row_slot(
		row_nodes,
		create_action_button_from_spec(model.kick_match_action),
		0.65,
		0.42
	)

	if lobby_context.is_teams_mode then
...
local function create_match_lobby_players_body_definition()
	...
	return {
		...
		nodes = {
			{
				n = G.UIT.C,
				config = { align = "tm", minw = lobby_context.is_teams_mode and 16.7 or 14.7, padding = 0.02 },
				nodes = player_rows,
			},
		},
	}
...
function MP.UI.refresh_match_lobby_info_players()
	...
	local players_body = G.OVERLAY_MENU:get_UIE_by_ID(MATCH_LOBBY_INFO_PLAYERS_BODY_ID)
	if not (players_body and players_body.config) then
		return false
	end

	return MP.UI.UTILS.replace_config_object(players_body, create_match_lobby_players_body_object(players_body), {
		recalculate_uie = true,
		recalculate_target = players_body.UIBox or G.OVERLAY_MENU,
	})
end
