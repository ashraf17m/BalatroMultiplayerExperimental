...
function view_model.cycle_view_target()
	local players, _, current_index = MP.UI.get_view_target_state()
	if #players == 0 then
		return false
	end
	...
	local next_index = (current_index % #players) + 1
	end_game_view.target_index = next_index
	MP.UI.request_end_game_view_target(players[next_index])
	return true
end
...
function view_model.toggle_players_jokers()
	...
	if end_game_view.jokers_text == localize("k_enemy_jokers") then
