...
local ui_state_store = load_required_service(
	"multiplayer/runtime/ui_state_store.lua",
...
function ui_api.add_nemesis_info(info_queue)
	...
	local opponent = MP.get_nemesis_lobby_player and MP.get_nemesis_lobby_player() or nil
...
function ui_api.refresh_active_pvp_player_list()
	if
		MP.is_pvp_boss
		and MP.is_pvp_boss()
		and ui_api.refresh_ffa_table
	then
		ui_api.refresh_ffa_table()
		return true
	end
