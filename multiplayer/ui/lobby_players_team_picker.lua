...
function lobby_players_ui.create_team_picker_definition(player_id)
	...
	local create_button_rows_from_specs = ROW_LAYOUT.create_button_rows_from_specs
	local picker_model = MP.build_lobby_team_picker_model and MP.build_lobby_team_picker_model(player_id) or nil
	local picker_rows = create_button_rows_from_specs((picker_model and picker_model.team_choice_rows) or {})
	...
	return create_UIBox_generic_options({
		...
		contents = {
			...
			},
			{
				...
				config = { align = "cm", padding = 0.08 },
				nodes = {
					create_button_from_spec((picker_model and picker_model.back_action) or {
...
BALATRO.set_ui_function("view_player_team_picker", function(e)
	...
	if not player_id or not (MP.can_edit_lobby_player_team and MP.can_edit_lobby_player_team(player_id)) then
		return
	end
...
BALATRO.set_ui_function("choose_player_team", function(e)
	...
	local applied_team_id = MP.apply_local_lobby_team_choice and MP.apply_local_lobby_team_choice(player_id, team_id) or nil
...
BALATRO.set_ui_function("toggle_player_team_lock", function(e)
	...
	if MP.toggle_local_lobby_player_team_lock then
		next_locked = MP.toggle_local_lobby_player_team_lock(player_id)
	end
