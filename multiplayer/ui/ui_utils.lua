...
-- Legacy helper still used by older ui/* surfaces that have not been migrated yet.
function MP.UI.UTILS.create_blank(minw, minh, node_type)
	return {
		n = node_type or G.UIT.R,
		config = {
			minw = minw or 0,
			minh = minh or 0,
		},
...
-- Legacy helper still used by older ui/* surfaces that expect a DynaText object builder.
function MP.UI.UTILS.create_dynatext(text, config)
	local dynatext_config = {}

	for key, value in pairs(config or {}) do
		dynatext_config[key] = value
	end

	if dynatext_config.string == nil then
		if type(text) == "table" then
			dynatext_config.string = text
		else
			dynatext_config.string = { { string = tostring(text or "") } }
		end
	end

	return DynaText(dynatext_config)
end
...
local function open_overlay_message_rows(message_rows, no_back)
	...
	BALATRO.open_overlay_menu({
		definition = create_UIBox_generic_options({
			...
			no_esc = no_back,
			contents = {
				MP.UI.UTILS.create_column({ align = "cm", padding = 0.2 }, message_rows),
			},
		}),
	})
end
