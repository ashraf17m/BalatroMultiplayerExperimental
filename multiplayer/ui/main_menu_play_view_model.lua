...
local function append_multiplayer_lobby_create_buttons(contents)
	...
	append_node(contents, create_play_button("b_create_1v1_lobby", G.C.GREEN, "create_1v1_lobby"))
	append_node(contents, create_play_button("b_create_group_lobby", G.C.BLUE, "create_group_lobby"))
end
...
function view_model.build_play_options_contents()
	if not G.SETTINGS.tutorial_complete or G.SETTINGS.tutorial_progress ~= nil then
		return {
			create_play_button("b_singleplayer", G.C.BLUE, "setup_run_singleplayer"),
			{
				n = G.UIT.R,
				config = {
					align = "cm",
	...
	local contents = {
		...
		create_play_button("b_sp_with_ruleset", G.C.ORANGE, "setup_run_singleplayer"),
	}
...
function G.UIDEF.override_main_menu_play_button()
	return view_model.create_play_options_overlay()
end

function G.UIDEF.create_UIBox_join_lobby_button()
	return view_model.create_join_lobby_overlay()
end

function G.UIDEF.weekly_interrupt()
	return view_model.create_weekly_interrupt_overlay()
end
