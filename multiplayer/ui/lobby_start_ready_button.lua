...
function MP.UI.create_lobby_main_button(text_scale)
	local button_state = MP.get_lobby_main_button_state()
	...
	if button_state.mode == "guest_ready_pending" then
		return create_lobby_main_disableable_button(text_scale, {
			button = "lobby_ready_up",
			...
			disabled_text = button_state.is_ready and localize("b_unready") or localize("b_ready"),
			enabled = false,
		})
	end
