...
local function build_lobby_team_picker_choice_rows(player_id, current_team)
	...
	for team_idx = 1, MP.MAX_TEAMS do
		...
		current_row.buttons[#current_row.buttons + 1] = {
			...
			label = MP.get_team_short_display_name(team_idx),
			minw = 1.75,
			minh = 0.62,
			scale = 0.45,
...
function MP.build_lobby_team_picker_model(player_id, opts)
	...
	local row_model = build_lobby_player_row_model(player, MP.get_lobby_player_display_index(player_id, {
...
function MP.build_lobby_player_surface_model(player, index, surface, opts)
	local options = opts or {}
	local lobby_context = options.lobby_context or (MP.get_lobby_state_context and MP.get_lobby_state_context()) or {}
	...
	local show_team_lane = not not (lobby_context.is_teams_mode and (surface == "info" or surface == "players"))
	...
	row_model.team_lane_spec = show_team_lane and (
		team_lane_interactive and {
			...
			label = MP.get_team_short_display_name(row_model.team),
			...
			kind = "chip",
			text = MP.get_team_short_display_name(row_model.team),
			colour = row_model.team_colour,
			text_colour = G.C.WHITE,
			minw = 1.15,
			scale = 0.45,
