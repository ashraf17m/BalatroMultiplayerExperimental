...
MP.UI.FFA_FULL_WINDOW = 6

function MP.UI.get_ffa_standings_ui_state()
	local player_list_runtime = MP.UI.get_player_list_runtime()
	player_list_runtime.ffa_standings = player_list_runtime.ffa_standings or {
		scroll_index = 1,
	}
	return player_list_runtime.ffa_standings
end

function MP.UI.reset_ffa_standings_ui_state()
	local player_list_runtime = MP.UI.get_player_list_runtime()
	player_list_runtime.ffa_standings = {
		scroll_index = 1,
	}
end

local function get_ffa_rank_theme(rank, is_self, pvp_col)
	if rank == 1 then
		return G.C.GOLD, "LEADER"
	elseif rank == 2 then
		return G.C.BLUE, "SECOND"
	elseif rank == 3 then
		return G.C.GREEN, "THIRD"
	elseif is_self then
		return G.C.GOLD, "YOU"
	end
	return darken(pvp_col, 0.15), nil
end
...
local function create_ffa_compact_entry(player, pvp_col)
	local accent = select(1, get_ffa_rank_theme(player.rank, player.is_self, pvp_col))
