...
local function should_animate_lobby_options_refresh(options)
	if not (MP.LOBBY and MP.LOBBY.is_host) then
		return true
	end

	return type(options) == "table" and options.different_seeds ~= nil
end
...
function MP.UI.refresh_lobby_options_tab(options)
	...
	if not should_animate_lobby_options_refresh(options) then
		return false
	end

	if not (G.FUNCS and G.FUNCS.lobby_options) then
		return false
	end

	G.FUNCS.lobby_options({
		config = { preserve_active_lobby_options_tab = true },
	})
	return true
end
...
function MP.UI.create_lobby_options_tab()
	return MP.UI.create_lobby_option_page(
		MP.UI.build_lobby_option_controls(MP.UI.LOBBY_OPTION_TAB_SPECS.options),
		4
	)
end
...
function MP.UI.create_gameplay_options_tab()
	return MP.UI.create_lobby_option_page(
		MP.UI.build_lobby_option_controls(MP.UI.LOBBY_OPTION_TAB_SPECS.gameplay),
		3
	)
end
	...
	local nodes = {
		{
			...
			nodes = MP.UI.build_lobby_option_controls(MP.UI.LOBBY_OPTION_TAB_SPECS.modifiers),
		},
	}

	return MP.UI.create_lobby_option_page(nodes, 6)
end
