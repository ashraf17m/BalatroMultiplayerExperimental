...
local function build_object_grid(obj_ids, object_table, objects_per_row, object_constructor, wrap_as_object)
	local grid = {}
	local row_nodes = {}
	...
	for idx, obj_id in ipairs(obj_ids) do
		...
		table.insert(row_nodes[row_index], {
			...
			nodes = {
				wrap_as_object and { n = G.UIT.O, config = { object = object_constructor(object_table[obj_id]) } }
					or object_constructor(object_table[obj_id]),
			},
		})
	end
...
local function build_other_tab_grid(args)
	...
	local tag_grid = build_object_grid(args.obj_ids.tags, G.P_TAGS, 4, tag_constructor)
	local blind_grid = build_object_grid(args.obj_ids.blinds, G.P_BLINDS, 3, blind_constructor, true)
