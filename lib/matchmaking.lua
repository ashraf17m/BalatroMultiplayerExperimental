...
function MP:generate_hash()
	...
	local unlocked = MP.UTILS.unlock_check()
	table.insert(mod_data, 1, "unlocked=" .. tostring(unlocked))
...
MP.HOOKS.register_method_hook(Game, "Game", "update", "mp.matchmaking.generate_hash", {
	after = function()
		if not hash_generated and MP.PLATFORM.SMODS.is_booted() then
			...
			if hash_generated and MP.HOOKS and MP.HOOKS.unregister_method_hook then
				MP.HOOKS.unregister_method_hook(Game, "update", "mp.matchmaking.generate_hash")
			end
		end
	end,
})
...
encrypt_ID = function()
	-- Legacy wire compatibility constant.
	-- The server and connecting clients parse "encryptID=<number>" from MP.MOD_STRING,
	-- but no matchmaking filtering or validation is performed on this value.
	return 1
end
...
MP.UTILS.encrypt_ID = encrypt_ID
