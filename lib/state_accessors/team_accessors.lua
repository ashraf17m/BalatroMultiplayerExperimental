...
function MP.get_team_short_display_name(team_idx)
...
function MP.can_edit_lobby_player_team(player_id, opts)
...
function MP.apply_local_lobby_team_choice(player_id, team_id, opts)
	local numeric_team_id = tonumber(team_id)
	if not player_id or not numeric_team_id or not (MP.can_edit_lobby_player_team and MP.can_edit_lobby_player_team(player_id, opts)) then
		return nil
	end
...
function MP.toggle_local_lobby_player_team_lock(player_id, opts)
...
function MP.get_next_local_self_team_choice(opts)
	local options = opts or {}
	local lobby_context = options.lobby_context or (MP.get_lobby_state_context and MP.get_lobby_state_context()) or {}
	if not lobby_context.is_teams_mode or lobby_context.match_in_progress then
		return nil
	...
	local self_player = lobby_context.self_player or nil
	if not self_player then
		return nil
	end

	local current_team = self_player.team or 1
	return (current_team % MP.MAX_TEAMS) + 1
end
