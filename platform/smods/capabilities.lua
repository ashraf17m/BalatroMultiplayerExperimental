...
function MP.PLATFORM.SMODS.get_version()
	return tostring(SMODS and SMODS.version or "")
end
...
function MP.PLATFORM.SMODS.has_optional_feature(feature_path)
	local current = SMODS and SMODS.optional_features or nil
	if not current then
		return false
	end

	for segment in tostring(feature_path or ""):gmatch("[^%.]+") do
