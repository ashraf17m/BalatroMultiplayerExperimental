...
local function calculate_current_round_score()
	...
	if _G.hand_chips ~= nil and _G.mult ~= nil then
		return CALC.mul_values(_G.hand_chips, _G.mult)
	end
...
local function run_real_evaluate_play(ctx)
	if not is_run_ready() then return fail("game is not in a playable run state") end
	if not ctx.full_hand or #ctx.full_hand == 0 then return CALC.exact_result(0, 0) end
	...
	if type(CALC.run_pre_score_effects) == "function" then
		...
		if not pre_score_ok then return fail(pre_score_err or "blind pre-play failed") end
	end

	local _, evaluate_err = call_or_fail("G.FUNCS.evaluate_play dry run", G.FUNCS.evaluate_play)
	if evaluate_err then return fail(evaluate_err) end
	if type(CALC.finish_dry_scoring_coroutine) == "function" then
		local scoring_done, scoring_err = CALC.finish_dry_scoring_coroutine()
		if not scoring_done then return fail("scoring coroutine failed: " .. tostring(scoring_err)) end
	end

	local score, score_err = calculate_current_round_score()
	if score == nil then return fail(score_err) end
