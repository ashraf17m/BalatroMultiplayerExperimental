...
function CALC.finish_dry_scoring_coroutine(limit)
	...
	if not scoring_coroutine then return true end
	if type(coroutine) ~= "table" or type(coroutine.status) ~= "function" then return true end
	if coroutine.status(scoring_coroutine) == "dead" then
	...
	local resumes = 0
	while G.SCORING_COROUTINE and coroutine.status(G.SCORING_COROUTINE) ~= "dead" do
		resumes = resumes + 1
		if resumes > limit then return nil, "scoring coroutine did not finish" end

		local ok, err = coroutine.resume(G.SCORING_COROUTINE)
