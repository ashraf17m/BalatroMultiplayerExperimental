...
local function install_dry_event_manager(replacements)
	...
	G.E_MANAGER.add_event = function(_, event)
		if CALC.dry_run_queue_events then dry_events[#dry_events + 1] = event end
		return event
	end
	...
	CALC.flush_dry_events = function(limit)
		...
		while dry_event_head <= #dry_events do
			...
			if processed > limit then return nil, "dry event limit reached" end
...
function CALC.run_pre_score_effects()
	...
	if not ok then return nil, "blind pre-play failed: " .. tostring(err) end
	if not flush_ok then return nil, "blind pre-play events failed: " .. tostring(flush_err) end
	return true
end
