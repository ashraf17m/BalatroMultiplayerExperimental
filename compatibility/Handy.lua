...
function MP.NETWORKING_INTERNAL.handle_handy_mp_extension_lobby_enabled(enabled)
	MP.LOBBY = MP.LOBBY or {}
	MP.LOBBY.config = MP.LOBBY.config or {}
	MP.LOBBY.config.handy_mp_extension_enabled = not not enabled

	if Handy then
		Handy.mp_extension_lobby_enabled = not not enabled
	end

	if sendTraceMessage then
		sendTraceMessage(
			"Client got handyMPExtensionLobbyEnabled message: "
				.. " (enabled: "
				.. tostring(enabled)
				.. ")"
