...
local function vanilla_key_for_sandbox(sandbox_key, group)
	local override = VANILLA_KEY_OVERRIDES[sandbox_key]
	if override ~= nil then
		return override or nil
	end

	if group == "extra_credit" then
		return nil
	end
...
local function sandbox_joker_mapping(sandbox_key, active, group)
	return {
		sandbox = sandbox_key,
		vanilla = vanilla_key_for_sandbox(sandbox_key, group),
		active = active,
		group = group,
	}
end
...
MP.Layer("sandbox", {
	...
	on_apply_bans = function()
		if apply_sandbox_bans then apply_sandbox_bans() end
	end,
})
