...
local function apply_smallworld_bans()
...
MP.Layer("smallworld", {
	on_apply_bans = apply_smallworld_bans,
})
...
local function smallworld_replacements_active()
	return MP.is_layer_active("smallworld") and not MP.legacy_smallworld()
end
...
function add_joker(joker, edition, silent, eternal)
	if MP.is_layer_active("smallworld") and is_banned_key(joker) then
		local pool
		local pool_key
...
MP.HOOKS.register_method_hook(Card, "Card", "apply_to_run", "mp.ruleset.smallworld.replace_banned_voucher", {
	before = function(ctx, self)
		...
		if MP.is_layer_active("smallworld") and not self and center and is_banned_key(center.key) then
